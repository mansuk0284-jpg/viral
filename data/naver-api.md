...
> ## ⛔ 2026-08-12 확인 — 검색 API 신규 발급 중단
>
> 네이버 개발자센터 애플리케이션 등록의 `사용 API` 목록에서 **검색이 사라졌다.**
> 남은 항목: 네이버 로그인 / 인증서 / 전자문서 / 페이 배송지 / 카페 / 캘린더 / 캡차(음성·이미지).
> 목록의 `카페`는 **자기 카페 글쓰기용**(네이버 로그인 연동 필수)이라 후기 검색에 못 쓴다.
>
> 따라서 아래 1~3절(키 발급·`naver-search.ps1`·`naver_api_collect.py`)은
> **기존 키 보유자만 유효**하다. 신규로는 동작시킬 수 없다.
>
> **대체 경로 → `scripts/naver_cafe_scraper.py` (로컬 Playwright).**
> `VIRAL_CLUBID` 환경변수로 대상 카페를 바꿔 다이렉트결혼준비와 같은 파이프라인
> (`menus` → `board --menu-id N --read-body --cumulative`)을 그대로 쓴다.
> 본문 열람은 로그인 세션 필요 — `login` 서브커맨드로 창을 띄우고 **사용자가 직접 로그인**한다.
