...
# 길이 동률의 2차 정렬을 사전순으로 못박는다 — set 순회는 해시 시드에 따라 달라져
# 같은 census 로 두 번 돌리면 매장 건수가 ±수 건 흔들렸다(2026-08-26 실측)
BRANCH_TOKENS = sorted({w for ws in SIDO.values() for w in ws}, key=lambda w: (-len(w), w))

# ── 비백화점 유통 마스킹 (2026-08-26) ─────────────────────────────────
# "롯데하이마트 메가스토어 센텀시티점" 류 문구가 '롯데'(체인)+'센텀'(지점)으로
# 백화점 매장에 오귀속되던 문제 — 유통·가구 상호와 그에 붙은 지점 표기를
# 매장 매칭 전에 통째로 걷어낸다. 같은 글의 진짜 백화점 언급은 그대로 남는다.
# 패턴별 실측(census 12.1만·blog 3.1만)으로 다듬었다:
#  - 지점 표기가 뒤(…센텀시티점)뿐 아니라 글루로 앞(#롯데잠실하이마트)에도 붙는다
#  - '…백화점' 토큰도 유통 뒤에 붙으면 그 유통의 소재지 표기다(안산롯데백화점) — 함께 걷는다
#  - '이마트'는 '하이마트' 안에서도 매치되므로 (?<!하) 가드
#  - 무풍갤러리·갤럭시·AKOR·비롯 = 체인 별칭('갤러리'·'갤'·'AK'·'롯')의 실측 오탐원
_ND_TOKEN = r"[가-힣A-Za-z0-9]{1,10}\s*(?:점|매장)"
NON_DEPT_RE = re.compile(
    "(?:"
    # 하이마트 — '#롯데잠실하이마트'처럼 지점이 앞에 글루로 붙는 표기까지 통째로
    r"(?:롯데\s*)?[가-힣A-Za-z0-9]{0,8}하이마트"
    r"|(?:롯데\s*)?[가-힣A-Za-z0-9]{0,6}피트인"   # 하이마트 피트인 포맷(#산본롯데피트인)
    r"|전자랜드"
    r"|커넥트현대"           # 현대의 몰형 포맷 — 분석 명부 밖, '현대+도시' 오탐 방지
    r"|롯데\s*마트"
    r"|롯데\s*몰"
    r"|(?:롯데|신세계|현대)?\s*(?:프리미엄\s*)?아울렛"
    r"|(?<!하)이마트(?:\s*트레이더스)?"
    r"|트레이더스"
    r"|홈플러스"
    r"|코스트코"
    r"|한샘"
    r"|일렉트로\s*마트"
    r"|무풍\s*갤러리"        # 삼성 에어컨 라인업명 — '갤러리' 체인 별칭 오탐 방지
    r"|갤럭시"               # 휴대폰명 — '갤' 체인 별칭 오탐 방지
    r"|AKOR"                 # LG 모델 색상코드 — 'AK' 체인 별칭 오탐 방지
    r"|비롯"                 # '비롯한'의 '롯' — '롯' 체인 별칭 오탐 방지
    ")"
    r"(?:[\s/(]*(?:메가스토어|커넥트현대|피트인)[)/]?)?"   # 매장 브랜드 수식어
    rf"(?:[\s/(]*{_ND_TOKEN}\)?)?"                          # 뒤따르는 지점 표기
    r"(?:\s*\([가-힣A-Za-z0-9]{1,8}\))?")                   # 지점 뒤 괄호 주석 예:(둔산)
...
# 부정(주의) 판정 — 낱말 단독 매치는 오탐이 컸다(실측 2026-08-26):
#   후회 987건 중 650건이 "후회 없으실 거예요"류 긍정, 하자 709건 중 9할이
#   "졸업하자" 같은 청유 어미, '지연'은 사람 이름, "늦게까지 상담"은 칭찬.
# 그래서 ①부정의 부정(없/않/안)은 제외 ②동형이의어는 명사·동사 용법만 잡는다.
NEG_RE = re.compile(
    "불친절|최악|짜증|환불"
    "|실망(?!\\s*(안|하지|시키지))"
    "|취소(?!\\s*없)"
    "|하자(?:가|를|\\s*있|\\s*발생|보수)"      # '-하자' 청유 어미 오탐 방지: 명사 용법만
    "|고장(?!\\s*없|\\s*나도|\\s*안)"
    "|지연(?:되|돼|됐|으로)"                    # 사람 이름 '지연' 오탐 방지
    "|늦게(?!까지)"
    # '기다렸' 단독은 "예약 안 했으면 기다렸겠다"(가정·칭찬)까지 잡는다 — 불만 문맥만
    "|(한참|오래|계속|헛)\\s*기다렸|기다렸는데"
    # '안와' 단독은 "감이 안와서" 오탐 — 배송·연락류 명사가 앞설 때만
    "|(연락|배송|설치|전화|기사)[가이도]?\\s*안\\s*와"
    "|불만(?!\\s*없)"
    "|후회(?!\\s*없|는\\s*없|가\\s*없|하지\\s*않|\\s*안)"
)
...
SPECIAL_STORES = [
    ...
    (["롯데센텀", "롯데 센텀"], "롯데 센텀시티"),
    (["신세계센텀", "신세계 센텀", "센텀시티", "센텀 시티", "부산센텀", "센텀신세계"], "신세계 센텀시티"),
    # 구체 지점(광복·동래·서면)이 도시 광역 표기("부산 롯데"류)보다 먼저 와야 한다 —
    # "부산 롯데동래점"이 '부산 롯데' 부분문자열로 부산본점에 먹히는 오분류가 있었다(2026-08-26 실측)
...
ALIAS_FIX = {
    "신세계 아산": "신세계 천안",       # 신세계 천안아산점 — 명부 정본은 천안
    "갤러리아 아산": "갤러리아 천안",   # 갤러리아센터시티(천안아산) — 명부 갤천안
...
def dept_store_of(txt):
    ...
    txt = NON_DEPT_RE.sub(" ", txt)       # 0) 비백화점 유통 문구(지점 표기 포함) 제거
    ...
    for chain, alias in CHAINS.items():   # 2) 체인 + 실제 지점 토큰
        for a in alias:
            ...
            w0 = max(0, i - 10)
            window = txt[w0:i + 14]
            c0, c1 = i - w0, i - w0 + len(a)      # 윈도 안 체인 위치
            # 후보가 여럿이면 체인과 가장 가까운 토큰이 지점이다 —
            # "안양 롯데 평촌롯데백화점"은 평촌(도시=안양, 지점=평촌).
            # 동거리면 체인 뒤쪽 우선(지점은 관용상 체인 뒤에 온다), 그다음 긴 토큰.
            best = None
            for b in BRANCH_TOKENS:
                ...
                if j < 0:
                    continue
                # 문맥은 윈도가 아니라 **원문 기준**으로 본다 — 윈도(±10/+14)가
                # "안동욱 매니저"의 호칭을 잘라 이름 가드가 헛돌았다(실측)
                aj = w0 + j
                nxt = txt[aj + len(b):aj + len(b) + 8]
                prv = txt[aj - 1:aj] if aj > 0 else ""
                # ① 사람 이름 가드 — "김수완 매니저"·"안동욱 명장"의 이름 조각이
                #    지점으로 잡히던 오탐(2026-08-26 실측 10건+)
                if re.match(r"[가-힣]{0,2}\s*(매니저|매니져|명장|프로|점장|부점장|상담사|디렉터|팀장|부장|님)", nxt):
                    continue
                # ② 낱말 내부 조각 가드 — "혼수완료"의 '수완'처럼 앞글자가 한글에
                #    붙어 있으면 '점'으로 이어질 때만 지점으로 본다("롯데동래점"은 유지)
                if prv and "가" <= prv <= "힣" and not nxt.startswith("점"):
                    continue
                after = j >= c1
                gap = j - c1 if after else c0 - (j + len(b))
                # 한국어 표기는 "도시 + 체인 + 지점점" — 체인 **뒤** 토큰이 지점이고,
                # '점' 접미가 붙은 토큰이 그중에서도 지점이다("안양평촌점"의 평촌).
                # 근접(gap)만 보면 앞의 도시가 뒤의 지점을 이긴다(성남 AK 분당점 실측)
                key = (0 if after else 1, 0 if nxt.startswith("점") else 1, max(gap, 0), -len(b))
