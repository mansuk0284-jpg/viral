...
■ 공식치(official) — 통계청(국가데이터처) 인구동향 보도자료. 수집 경로:
  ...
  - 2026-06 21,075(+14.0%) / 2025-06 18,482(기사 병기 실측)
    https://newseyes.net/news/11794 (2026-09-08 확보)
...
■ 추정치(est) — 미발표 월. 방법을 그대로 각주로 화면에 알린다:
  ...
  - 2026-07 이후 = 전년 동월값 × 2026 확정월 평균 증감률(+5.8%)
"""
import io
import json
import os
...
OFFICIAL = {
    ...
    "2026-04": 20622, "2026-05": 20368, "2026-06": 21075,
    "2025-01": 20142, "2025-02": 19370, "2025-03": 19181,
    "2025-04": 18920, "2025-05": 21760, "2025-06": 18482, "2025-09": 18462,
    "2024-02": 16947, "2024-03": 17203, "2024-09": 15372,
}
ANNUAL = {2021: 192507, 2022: 191690, 2023: 193657, 2024: 222412, 2025: 240326}
YOY_2026 = 1.058          # 2026 확정 6개월 평균 증감률(+12.4 −4.2 +10.1 +9.0 −6.4 +14.0)

# ── 시도 비중(추정) — 지역 화면용(2026-08-27 확장 지시) ─────────────────
# 시도별 '월별 건수'는 공개 텍스트로 구할 수 없어(보도자료 첨부 HWP/PDF 안 표),
# 공식 근거 두 가지를 곱해 비중을 만든다:
#   ① 행안부 주민등록 시도 인구(2025, 천 명)  https://jumin.mois.go.kr/
#   ② 통계청 시도별 조혼인율 확보분 — 2025: 대전 6.1·서울 5.3·세종 5.1(전국 4.7)
#      2024: 경기 4.6·부산 3.5·경남 3.5(전국 4.4)
#      https://mods.go.kr/board.es?mid=a10301020300&act=view&bid=204&list_no=444103
#      https://www.korea.kr/news/policyNewsView.do?newsId=156679907
#   가중치 = 인구 × (조혼인율/전국율). 율 미확보 시도는 전국 평균 성향(=1.0)으로
#   두므로 **추정**이다 — 화면 라벨·툴팁에 그대로 밝힌다.
REGION_POP = {  # 천 명
    "서울": 9330, "부산": 3250, "대구": 2360, "인천": 3030, "광주": 1400,
    "대전": 1440, "울산": 1090, "세종": 392, "경기": 13690, "강원": 1520,
    "충북": 1590, "충남": 2140, "전북": 1740, "전남": 1770, "경북": 2540,
    "경남": 3220, "제주": 670,
}
REGION_RATE = {  # 시도 조혼인율 / 전국 조혼인율 (공식 확보분만)
    "대전": 6.1 / 4.7, "서울": 5.3 / 4.7, "세종": 5.1 / 4.7,
    "경기": 4.6 / 4.4, "부산": 3.5 / 4.4, "경남": 3.5 / 4.4,
}
...
def main():
    ...
    w = {r: REGION_POP[r] * REGION_RATE.get(r, 1.0) for r in REGION_POP}
    tot_w = sum(w.values())
    region_share = {r: round(w[r] / tot_w, 5) for r in w}

    data = {
        "months": months,
        "regionShare": region_share,
        "regionNote": ("시도 혼인은 추정 — 전국 공식 월별 × 시도 비중"
                       "(주민등록 인구 × 공식 조혼인율 보정, 율 미확보 시도는 전국 평균 성향)"),
        "src": "통계청(국가데이터처) 인구동향 월별 보도자료 · 연간 확정치(KOSIS)",
        "estNote": ("미발표 월은 추정 — 2021~2025는 연간 공식치의 잔여를 균등 배분, "
                    "2026년 7월 이후는 전년 동월 × 발표월 평균 증감률(+5.8%)"),
    }
